...
const [scheduleListbyClasscode, setScheduleListbyClasscode] = useState<ScheduleDataType[]>([]);
const [scheduleListbyClassroom, setScheduleListbyClassroom] = useState<number[]>([]);//seçilen periodda classroom seçilince dolu olan slot idleri gelir. array
// const [assignTimeSlots, setAssignTimeSlots] = useState<ScheduleDataType[]>([]);
const [timeSlotList, setTimeSlotList] = useState<number[]>([]);
const [selectedData, setSelectedData] = useState<SelectedDataType>({classcode_id:null, person_limit: 0,teacher_id:null,period_id:null,classroom_id:null});
useEffect(() => {
getResponseList();
}, []);
...
const controleClasscode = (
  ...
  if (currentTimeSlotIds.length > 0) {
    ...
    const filteredIds = currentTimeSlotIds.filter(id => !usedTimeSlotIds.has(id));
...
const getSchedulebyClassroom=async()=>{
  ...
  const response = await fetch(
    ...
    const usedTimeSlotIds = [...new Set(data?.map(item => item.time_slot_id))];
...
const getTimeSlotandSchedulebyClasscode = async (programId: number) => {
  try {
    ...
    const dataSchedulebyClasscode: ScheduleDataType[] = await responseSchedulebyClasscode.json();
...
const handleClear=()=>{
  setSelectedData({classcode_id:null, person_limit: 0,teacher_id:null,period_id:null,classroom_id:null})
}
    return (
